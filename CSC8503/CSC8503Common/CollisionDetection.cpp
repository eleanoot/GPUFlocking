...
bool CollisionDetection::RayBoxIntersection(const Ray& r, const Vector3& boxPos, const Vector3& boxSize, RayCollision& collision) {
	...
	// Only check the 3 closest planes rather than all six.  
	for (int i = 0; i < 3; ++i)
	{
		// If ray going left, check only right side of box; going right, left side etc. 
		if (rayDir[i] > 0)
	...
	// Determine best intersection point along ray vector.
	Vector3 intersection = rayPos + (rayDir * bestT);

	const float epsilon = 0.0001f; // give some leeway in calcs

	// Is they ray actually on the box surface or not?
	for (int i = 0; i < 3; ++i)
	{
...
bool CollisionDetection::RayOBBIntersection(const Ray& r, const Transform& worldTransform, const OBBVolume& volume, RayCollision& collision) {
	...
	Matrix3 invTransform = Matrix3(orientation.Conjugate()); // conjugate nicer than inverting matrices!

	Vector3 localRayPos = r.GetPosition() - position;

	// Transform ray so its in the local space of the box. 
	Ray tempRay(invTransform * localRayPos, invTransform * r.GetDirection());
	// Raycast vs AABB at the origin. 
	bool collided = RayBoxIntersection(tempRay, Vector3(), volume.GetHalfDimensions(), collision);

	if (collided)
		// Transform collision point back into world space.
...
bool CollisionDetection::RaySphereIntersection(const Ray& r, const Transform& worldTransform, const SphereVolume& volume, RayCollision& collision) {
	...
	// Project sphere's origin onto ray direction vector.
	...
	// Determine collision point by moving collision point back along direction vector to touch surface of sphere instead.
...
// Take two objects and determine which collision detection function to use. 
bool CollisionDetection::ObjectIntersection(GameObject* a, GameObject* b, CollisionInfo& collisionInfo) {
	...
	// Bitwise op on enums- each entry is a power of 2, so will get a bitmask of the type of collision
	// If after the type still equates to a given type, they're both the same and order doesn't matter.
...
// Test if the objects are colliding
bool CollisionDetection::AABBTest(const Vector3& posA, const Vector3& posB, const Vector3& halfSizeA, const Vector3& halfSizeB) {
	// See if the distance between the objects on each axis is less than the sum of the box sizes on that axis.
	// Only colliding if true for all axis.
	Vector3 delta = posB - posA;
	Vector3 totalSize = halfSizeA + halfSizeB;
	// Absolute values used as may be either side of box B.
...
bool CollisionDetection::AABBIntersection(const AABBVolume& volumeA, const Transform& worldTransformA,
	...
	// Determine if the boxes are overlapping. 
	...
	if (overlap)
	{
		...
		// Determine the min and max position for each axis
		...
		// Use the min and max to calculate the amount of overlap on each axis
		float distances[6] =
		{
		...
		// Iterate through penetration extents and find the smallest value
		for (int i = 0; i < 6; i++)
		{
		...
		// Build the collision info- set collision points to the origin so they don't twist and stay axis aligned. 
