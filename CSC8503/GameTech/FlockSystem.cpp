...
void FlockSystem::InitPartitionInstance(OGLMesh* m, RenderObject* r)
{
	boidMesh = m;
	flock_member fm;
	// set up the flock member data
...
void FlockSystem::UpdatePartitionFlock(float dt)
{
	...
	indexShader->Execute(ceil(flockSize / WORK_GROUP_SIZE), 1, 1);
	glMemoryBarrier(GL_SHADER_STORAGE_BARRIER_BIT);
	indexShader->Unbind();
	...
	flockShader->Execute(ceil(flockSize / WORK_GROUP_SIZE), 1, 1);
	glMemoryBarrier(GL_SHADER_STORAGE_BARRIER_BIT);
	flockShader->Unbind();
...
void FlockSystem::UpdatePartitionInstance(float dt)
{
	// bind buffers for position in and out 
	obstacles[0]->UpdateObstacle(dt);
	obPtr[0].centre = obstacles[0]->GetTransform().GetWorldPosition();
	...
	glUniform1i(glGetUniformLocation(flockShader->GetProgramID(), "cellCount"), cellCount);
	glUniform2ui(glGetUniformLocation(flockShader->GetProgramID(), "cellCounts"), cellCounts.x, cellCounts.y);
	glUniform1i(glGetUniformLocation(flockShader->GetProgramID(), "worldSize"), worldBounds.x * worldBounds.y);
