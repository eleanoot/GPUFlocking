		...
		*/
		if (sm->vertIndices.empty()) {
			...
			for (unsigned int j = 0; j < sm->vertIndices.size(); ++j) {
				m->positions.emplace_back(inputVertices[sm->vertIndices[j] - 1]);
				m->indices.emplace_back(j);
			}

			if (!sm->texIndices.empty()) {
				for (unsigned int j = 0; j < sm->texIndices.size(); ++j) {
					m->texCoords.emplace_back(inputTexCoords[sm->texIndices[j] - 1]);
				}
			}
	...
	while (!f.eof()) {
		...
		if (currentLine == MTLNEW) {
			...
			if (!currentMTL.diffuse.empty()) {
				currentMTL.diffuseNum = ((OGLTexture*)TextureLoader::LoadAPITexture(string(TEXTUREDIR + currentMTL.diffuse).c_str()))->GetObjectID();
				//SOIL_load_OGL_texture(string(TEXTUREDIR + currentMTL.diffuse).c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_INVERT_Y | SOIL_FLAG_TEXTURE_REPEATS);
			}
		}
			...
			if (!currentMTL.bump.empty()) {
				currentMTL.bumpNum = ((OGLTexture*)TextureLoader::LoadAPITexture(string(TEXTUREDIR + currentMTL.bump).c_str()))->GetObjectID();
				//SOIL_load_OGL_texture(string(TEXTUREDIR + currentMTL.bump).c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_INVERT_Y | SOIL_FLAG_TEXTURE_REPEATS);
			}
		}
	}
